...
class AWSUtilsTestCase(TestCase):
    ...
    def test_get_code_upload_setup_meta_for_challenge_with_host_credentials(self, mock_get_challenge_model, mock_get_cluster):
        ...
        mock_challenge_evaluation_cluster.node_group_arn_role = 'node_group_arn_role'
    ...
    @mock.patch('challenges.utils.get_challenge_model')
    @mock.patch('challenges.aws_utils.VPC_DICT', {
        "SUBNET_1": "vpc_subnet1",
        "SUBNET_2": "vpc_subnet2",
        "SUBNET_SECURITY_GROUP": "vpc_sg"
    })
    @mock.patch('challenges.aws_utils.settings')
    def test_get_code_upload_setup_meta_for_challenge_without_host_credentials(self, mock_settings, mock_get_challenge_model):
        # Mock the return value of get_challenge_model
        mock_challenge = mock_get_challenge_model.return_value
...
class TestCreateServiceByChallengePk:
    def test_create_service_success(self, mock_client, mock_challenge, client_token):
        mock_challenge.workers = None
        mock_challenge.task_def_arn = "valid_task_def_arn"

        response_metadata = {"HTTPStatusCode": HTTPStatus.OK}
        mock_client.create_service.return_value = {"ResponseMetadata": response_metadata}

        with patch('challenges.aws_utils.register_task_def_by_challenge_pk', return_value={"ResponseMetadata": response_metadata}):
            response = create_service_by_challenge_pk(mock_client, mock_challenge, client_token)

        assert response["ResponseMetadata"]["HTTPStatusCode"] == HTTPStatus.OK
        mock_challenge.save.assert_called_once()
        assert mock_challenge.workers == 1

    def test_create_service_client_error(self, mock_client, mock_challenge, client_token):
        mock_challenge.workers = None
        mock_challenge.task_def_arn = "valid_task_def_arn"

        mock_client.create_service.side_effect = ClientError(
            error_response={"Error": {"Code": "SomeError"}, "ResponseMetadata": {"HTTPStatusCode": HTTPStatus.BAD_REQUEST}},
            operation_name='CreateService'
        )

        with patch('challenges.aws_utils.register_task_def_by_challenge_pk', return_value={"ResponseMetadata": {"HTTPStatusCode": HTTPStatus.OK}}):
            response = create_service_by_challenge_pk(mock_client, mock_challenge, client_token)

        assert response["ResponseMetadata"]["HTTPStatusCode"] == HTTPStatus.BAD_REQUEST

    def test_service_already_exists(self, mock_client, mock_challenge, client_token):
    ...
    def test_register_task_def_fails(self, mock_client, mock_challenge, client_token):
        mock_challenge.workers = None
        mock_challenge.task_def_arn = None  # Simulate task definition is not yet registered

        register_task_response = {"ResponseMetadata": {"HTTPStatusCode": HTTPStatus.BAD_REQUEST}}

        with patch('challenges.aws_utils.register_task_def_by_challenge_pk', return_value=register_task_response):
            response = create_service_by_challenge_pk(mock_client, mock_challenge, client_token)
...
def test_update_service_success(mock_client, mock_challenge, num_of_tasks):
    ...
    response = update_service_by_challenge_pk(mock_client, mock_challenge, num_of_tasks)
...
def test_update_service_client_error(mock_client, mock_challenge, num_of_tasks):
    mock_challenge.queue = "dummy_queue"
    mock_challenge.task_def_arn = "valid_task_def_arn"

    mock_client.update_service.side_effect = ClientError(
        error_response={"Error": {"Code": "ServiceError"}, "ResponseMetadata": {"HTTPStatusCode": HTTPStatus.BAD_REQUEST}},
...
def test_update_service_force_new_deployment(mock_client, mock_challenge, num_of_tasks):
    mock_challenge.queue = "dummy_queue"
    mock_challenge.task_def_arn = "valid_task_def_arn"
    ...
    response = update_service_by_challenge_pk(mock_client, mock_challenge, num_of_tasks, force_new_deployment=True)
...
def test_delete_service_success_when_workers_zero(mock_challenge, mock_client):
    ...
    mock_challenge.task_def_arn = "valid_task_def_arn"  # Ensure task_def_arn is set to a valid string
    response_metadata_ok = {"ResponseMetadata": {"HTTPStatusCode": HTTPStatus.OK}}

    with patch('challenges.aws_utils.get_boto3_client', return_value=mock_client):
    ...
    mock_client.deregister_task_definition.assert_called_once_with(taskDefinition="valid_task_def_arn")


def test_delete_service_success_when_workers_not_zero(mock_challenge, mock_client):
    mock_challenge.workers = 3
    mock_challenge.task_def_arn = "valid_task_def_arn"
    response_metadata_ok = {"ResponseMetadata": {"HTTPStatusCode": HTTPStatus.OK}}

    with patch('challenges.aws_utils.get_boto3_client', return_value=mock_client):
        with patch('challenges.aws_utils.update_service_by_challenge_pk', return_value=response_metadata_ok):
            mock_client.delete_service.return_value = response_metadata_ok
...
def test_update_service_failure(mock_challenge, mock_client):
    mock_challenge.workers = 3
    response_metadata_error = {"ResponseMetadata": {"HTTPStatusCode": HTTPStatus.BAD_REQUEST}}

    with patch('challenges.aws_utils.get_boto3_client', return_value=mock_client):
        with patch('challenges.aws_utils.update_service_by_challenge_pk', return_value=response_metadata_error):
            response = delete_service_by_challenge_pk(mock_challenge)
...
def test_deregister_task_definition_failure(mock_challenge, mock_client):
    ...
    with patch('challenges.aws_utils.get_boto3_client', return_value=mock_client):
        ...
        mock_client.deregister_task_definition.side_effect = ClientError(
            error_response={"Error": {"Code": "DeregisterError"}, "ResponseMetadata": {"HTTPStatusCode": HTTPStatus.BAD_REQUEST}},
    ...
    assert response["ResponseMetadata"]["HTTPStatusCode"] == HTTPStatus.BAD_REQUEST
    mock_client.deregister_task_definition.assert_called_once_with(taskDefinition=mock_challenge.task_def_arn)
...
def test_delete_service_client_error(mock_challenge, mock_client):
    ...
    with patch('challenges.aws_utils.get_boto3_client', return_value=mock_client):
        mock_client.delete_service.side_effect = ClientError(
            error_response={"Error": {"Code": "DeleteServiceError"}, "ResponseMetadata": {"HTTPStatusCode": HTTPStatus.BAD_REQUEST}},
...
class TestServiceManager:
    ...
    def test_service_manager_updates_service(self, mock_client, mock_challenge):
        # Setup
        mock_challenge.workers = 1
        num_of_tasks = 5
        ...
        with patch('challenges.aws_utils.update_service_by_challenge_pk', return_value=response_metadata_ok) as mock_update:
            # Call the function
            response = service_manager(mock_client, mock_challenge, num_of_tasks=num_of_tasks, force_new_deployment=force_new_deployment)

            # Verify
            assert response == response_metadata_ok
            mock_update.assert_called_once_with(mock_client, mock_challenge, num_of_tasks, force_new_deployment)

    def test_service_manager_creates_service(self, mock_client, mock_challenge):
        # Setup
        mock_challenge.workers = None
        response_metadata_ok = {"ResponseMetadata": {"HTTPStatusCode": HTTPStatus.OK}}
        ...
        with patch('challenges.aws_utils.client_token_generator', return_value='mock_client_token'):
            with patch('challenges.aws_utils.create_service_by_challenge_pk', return_value=response_metadata_ok) as mock_create:
                ...
                # Verify
                assert response == response_metadata_ok
                mock_create.assert_called_once_with(mock_client, mock_challenge, 'mock_client_token')
...
class TestStopEc2Instance(unittest.TestCase):
    ...
    def test_stop_instance_success(self, mock_get_boto3_client):
        ...
        mock_ec2.stop_instances.return_value = {"StoppingInstances": [{"InstanceId": "i-1234567890abcdef0", "CurrentState": {"Name": "stopping"}}]}
        ...
        self.assertEqual(result['response'], {"StoppingInstances": [{"InstanceId": "i-1234567890abcdef0", "CurrentState": {"Name": "stopping"}}]})
        self.assertEqual(result['message'], "Instance for challenge 1 successfully stopped.")
    ...
    def test_instance_not_running(self, mock_get_boto3_client):
        ...
        # Checking the response
        self.assertEqual(result['error'], "Instance for challenge 1 is not running. Please ensure the instance is running.")
    ...
    def test_status_checks_not_ready(self, mock_get_boto3_client):
        ...
        # Checking the response
        self.assertEqual(result['error'], "Instance status checks are not ready for challenge 1. Please wait for the status checks to pass.")
    ...
    def test_instance_not_found(self, mock_get_boto3_client):
        ...
        # Checking the response
        self.assertEqual(result['error'], "Instance for challenge 1 not found. Please ensure the instance exists.")


class TestDescribeEC2Instance(unittest.TestCase):
    @patch('challenges.aws_utils.get_boto3_client')  # Mock the `get_boto3_client` function
    def test_describe_ec2_instance_success(self, mock_get_boto3_client):
        ...
        # Assert the result
        self.assertEqual(result, {"message": {"InstanceId": "i-1234567890abcdef0", "State": {"Name": "running"}}})
        mock_ec2.describe_instances.assert_called_once_with(InstanceIds=["i-1234567890abcdef0"])

    @patch('challenges.aws_utils.get_boto3_client')
    @patch('builtins.open', new_callable=mock_open, read_data="ec2_worker_script_content")
    def test_set_ec2_storage(self, mock_open, mock_get_boto3_client):
        ...
        mock_open.assert_called_once_with('/code/scripts/deployment/deploy_ec2_worker.sh')
    ...
    def test_set_worker_instance_type(self, mock_open, mock_get_boto3_client):
        ...
        result = create_ec2_instance(challenge, worker_instance_type='t3.medium')
    ...
    def test_set_worker_image_url(self, mock_open, mock_get_boto3_client):
        ...
        result = create_ec2_instance(challenge, worker_image_url='ami-12345678')
...
class TestStartEC2Instance(unittest.TestCase):
    ...
    def test_start_ec2_instance_success(self, mock_get_boto3_client):
        ...
        mock_response = {
            "Reservations": [
                {
                    "Instances": [
                        {"InstanceId": "i-1234567890abcdef0", "State": {"Name": "stopped"}}
                    ]
                }
            ]
        }
        mock_ec2.describe_instances.return_value = mock_response
        mock_start_response = {"StartingInstances": [{"InstanceId": "i-1234567890abcdef0", "CurrentState": {"Name": "pending"}}]}
    ...
    def test_start_ec2_instance_exception(self, mock_logger, mock_get_boto3_client):
        # Setup mock
        mock_ec2 = MagicMock()
        mock_get_boto3_client.return_value = mock_ec2
        ...
        mock_ec2.start_instances.side_effect = ClientError({"Error": {"Message": "Test Exception"}}, "StartInstances")
...
class TestRestartEC2Instance(unittest.TestCase):
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.logger')
    def test_restart_ec2_instance_success(self, mock_logger, mock_get_boto3_client):
        # Setup mock
        mock_ec2 = MagicMock()
        mock_get_boto3_client.return_value = mock_ec2
        mock_response = {"RebootingInstances": [{"InstanceId": "i-1234567890abcdef0"}]}
    ...
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.logger')
    def test_restart_ec2_instance_client_error(self, mock_logger, mock_get_boto3_client):
        # Setup mock
        mock_ec2 = MagicMock()
        mock_get_boto3_client.return_value = mock_ec2

        # Simulate ClientError
        error_response = {'Error': {'Code': 'InvalidInstanceID.NotFound', 'Message': 'The instance ID does not exist'}}
        mock_ec2.reboot_instances.side_effect = ClientError(error_response, 'RebootInstances')
        ...
        mock_logger.exception.assert_called_once_with(mock_ec2.reboot_instances.side_effect)


class TestTerminateEC2Instance(unittest.TestCase):
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.logger')
    def test_terminate_ec2_instance_success(self, mock_logger, mock_get_boto3_client):
        # Setup mock
        mock_ec2 = MagicMock()
        mock_get_boto3_client.return_value = mock_ec2
        mock_response = {"TerminatingInstances": [{"InstanceId": "i-1234567890abcdef0"}]}
    ...
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.logger')
    def test_terminate_ec2_instance_client_error(self, mock_logger, mock_get_boto3_client):
        # Setup mock
        mock_ec2 = MagicMock()
        mock_get_boto3_client.return_value = mock_ec2
        ...
        # Ensure the exception was logged
        mock_logger.exception.assert_called_once_with(mock_ec2.terminate_instances.side_effect)
...
class TestCreateEC2Instance(unittest.TestCase):
    ...
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('builtins.open', new_callable=mock_open, read_data="ec2_worker_script_content")
    def test_create_ec2_instance_success(self, mock_open, mock_get_boto3_client):
        # Setup mock
        mock_ec2 = MagicMock()
        mock_get_boto3_client.return_value = mock_ec2
        mock_response = {"Instances": [{"InstanceId": "i-1234567890abcdef0"}]}
    ...
    @patch('builtins.open', new_callable=mock_open, read_data="ec2_worker_script_content")
    @patch('challenges.aws_utils.logger')
    def test_create_ec2_instance_client_error(self, mock_logger, mock_open, mock_get_boto3_client):
        # Setup mock
        mock_ec2 = MagicMock()
        mock_get_boto3_client.return_value = mock_ec2

        # Simulate ClientError
        error_response = {'Error': {'Code': 'InvalidParameterValue', 'Message': 'The parameter value is invalid'}}
        ...
        with patch('challenges.aws_utils.aws_keys', {
...
class TestUpdateSQSRetentionPeriod(unittest.TestCase):
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.logger')
    def test_update_sqs_retention_period_success(self, mock_logger, mock_get_boto3_client):
        # Setup mock SQS client
        mock_sqs = MagicMock()
        mock_get_boto3_client.return_value = mock_sqs
        mock_sqs.get_queue_url.return_value = {'QueueUrl': 'https://sqs.us-west-1.amazonaws.com/123456789012/test_queue'}
        mock_sqs.set_queue_attributes.return_value = {'ResponseMetadata': {'HTTPStatusCode': 200}}
        ...
        mock_sqs.get_queue_url.assert_called_once_with(QueueName=challenge.queue)
    ...
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.logger')
    def test_update_sqs_retention_period_failure(self, mock_logger, mock_get_boto3_client):
        # Setup mock SQS client
        mock_sqs = MagicMock()
        mock_get_boto3_client.return_value = mock_sqs
        mock_sqs.get_queue_url.side_effect = ClientError(
            {'Error': {'Code': 'QueueDoesNotExist', 'Message': 'The queue does not exist'}},
            'GetQueueUrl'
        )
        ...
        # Assert the expected output
        self.assertEqual(result, {"error": "An error occurred (QueueDoesNotExist) when calling the GetQueueUrl operation: The queue does not exist"})
...
class TestStartWorkers(unittest.TestCase):
    ...
    def test_start_workers_debug_mode(self, mock_settings, mock_service_manager, mock_get_boto3_client):
        # Mock queryset
        challenge = MagicMock()
        challenge.pk = 1
    ...
    def test_start_workers_success(self, mock_settings, mock_service_manager, mock_get_boto3_client):
        ...
        # Ensure methods were called with expected arguments
        aws_keys = {'AWS_ACCOUNT_ID': 'x', 'AWS_ACCESS_KEY_ID': 'x', 'AWS_SECRET_ACCESS_KEY': 'x', 'AWS_REGION': 'us-east-1', 'AWS_STORAGE_BUCKET_NAME': 'evalai-s3-bucket'}
    ...
    @patch('challenges.aws_utils.service_manager')
    @patch('challenges.aws_utils.settings', DEBUG=False)
    def test_start_workers_failure(self, mock_settings, mock_service_manager, mock_get_boto3_client):
        # Setup mock ECS client
        mock_client = MagicMock()
        mock_get_boto3_client.return_value = mock_client
        mock_service_manager.return_value = {
    ...
    @patch('challenges.aws_utils.service_manager')
    @patch('challenges.aws_utils.settings', DEBUG=False)
    def test_start_workers_with_active_workers(self, mock_settings, mock_service_manager, mock_get_boto3_client):
    ...
    def test_stop_workers_debug_mode(self, mock_settings):
        ...
        # Assert the expected output
        expected_failures = [
            {"message": "Workers cannot be stopped on AWS ECS service in development environment", "challenge_pk": 1},
            {"message": "Workers cannot be stopped on AWS ECS service in development environment", "challenge_pk": 2}
        ]
    ...
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.service_manager')
    def test_stop_workers_success(self, mock_service_manager, mock_get_boto3_client, mock_settings):
        ...
        mock_service_manager.assert_called_with(mock_ec2, challenge=challenge2, num_of_tasks=0)
        self.assertEqual(mock_service_manager.call_count, 2)

    @patch('challenges.aws_utils.settings', DEBUG=False)
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.service_manager')
    def test_stop_workers_failure(self, mock_service_manager, mock_get_boto3_client, mock_settings):
        # Mock client and service manager response with error
        mock_ec2 = MagicMock()
        mock_get_boto3_client.return_value = mock_ec2
        mock_response = {"ResponseMetadata": {"HTTPStatusCode": HTTPStatus.BAD_REQUEST}, "Error": "Error stopping worker"}
        mock_service_manager.return_value = mock_response
        ...
        expected_failures = [
            ...
            {"message": "Please select challenges with active workers only.", "challenge_pk": 2}
        ]
        self.assertEqual(result, {"count": 0, "failures": expected_failures})

        # Ensure the service manager was called correctly
        mock_service_manager.assert_called_with(mock_ec2, challenge=challenge1, num_of_tasks=0)
        self.assertEqual(mock_service_manager.call_count, 1)

    @patch('challenges.aws_utils.settings', DEBUG=False)
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.service_manager')
    def test_stop_workers_no_active_workers(self, mock_service_manager, mock_get_boto3_client, mock_settings):
        ...
        # Assert the expected output
        expected_failures = [
            {"message": "Please select challenges with active workers only.", "challenge_pk": 1},
            {"message": "Please select challenges with active workers only.", "challenge_pk": 2}
        ]
        self.assertEqual(result, {"count": 0, "failures": expected_failures})
...
class TestScaleWorkers(unittest.TestCase):
    ...
    def test_scale_workers_debug_mode(self, mock_settings):
        ...
        # Assert the expected output
        expected_failures = [
            {"message": "Workers cannot be scaled on AWS ECS service in development environment", "challenge_pk": 1},
            {"message": "Workers cannot be scaled on AWS ECS service in development environment", "challenge_pk": 2}
        ]
        self.assertEqual(result, {"count": 0, "failures": expected_failures})
    ...
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.service_manager')
    def test_scale_workers_no_current_workers(self, mock_service_manager, mock_get_boto3_client, mock_settings):
        # Mock client and service manager response
        mock_ec2 = MagicMock()
        mock_get_boto3_client.return_value = mock_ec2
        ...
        # Assert the expected output
        expected_failures = [
            {"message": "Please start worker(s) before scaling.", "challenge_pk": 1},
            {"message": "Please start worker(s) before scaling.", "challenge_pk": 2}
        ]
        self.assertEqual(result, {"count": 0, "failures": expected_failures})
    ...
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.service_manager')
    def test_scale_workers_same_num_of_tasks(self, mock_service_manager, mock_get_boto3_client, mock_settings):
        # Mock client and service manager response
        mock_ec2 = MagicMock()
        mock_get_boto3_client.return_value = mock_ec2
        ...
        # Assert the expected output
        expected_failures = [
            {"message": "Please scale to a different number. Challenge has 5 worker(s).", "challenge_pk": 1},
            {"message": "Please scale to a different number. Challenge has 5 worker(s).", "challenge_pk": 2}
        ]
        self.assertEqual(result, {"count": 0, "failures": expected_failures})
    ...
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.service_manager')
    def test_scale_workers_success(self, mock_service_manager, mock_get_boto3_client, mock_settings):
        # Mock client and service manager response
        mock_ec2 = MagicMock()
        mock_get_boto3_client.return_value = mock_ec2
        mock_response = {"ResponseMetadata": {"HTTPStatusCode": HTTPStatus.OK}}
        ...
        # Ensure the service manager was called correctly
        mock_service_manager.assert_called_with(mock_ec2, challenge=challenge2, num_of_tasks=7)
        self.assertEqual(mock_service_manager.call_count, 2)

    @patch('challenges.aws_utils.settings', DEBUG=False)
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.service_manager')
    def test_scale_workers_failure(self, mock_service_manager, mock_get_boto3_client, mock_settings):
        # Mock client and service manager response with error
        mock_ec2 = MagicMock()
        mock_get_boto3_client.return_value = mock_ec2
        mock_response = {"ResponseMetadata": {"HTTPStatusCode": HTTPStatus.BAD_REQUEST}, "Error": "Error scaling workers"}
        mock_service_manager.return_value = mock_response
...
class TestScaleResources(unittest.TestCase):
    @patch('challenges.aws_utils.settings', DEBUG=False)
    @patch('challenges.aws_utils.get_boto3_client')
    def test_scale_resources_no_changes(self, mock_get_boto3_client, mock_settings):
        # Mock client
        mock_client = MagicMock()
        mock_get_boto3_client.return_value = mock_client
        # Mock challenge
        ...
        result = scale_resources(challenge, worker_cpu_cores=2, worker_memory=4096)
        # Assert the expected output
        expected_result = {
            "Success": True,
    ...
    @patch('challenges.aws_utils.settings', DEBUG=False)
    @patch('challenges.aws_utils.get_boto3_client')
    def test_scale_resources_no_task_def_arn(self, mock_get_boto3_client, mock_settings):
        # Mock client
        mock_client = MagicMock()
        mock_get_boto3_client.return_value = mock_client
        ...
        challenge.worker_memory = 4096
        # Call the function
        result = scale_resources(challenge, worker_cpu_cores=4, worker_memory=8192)
        # Assert the expected output
        expected_result = {
            "Error": "Error. No active task definition registered for the challenge {}.".format(challenge.pk),
            "ResponseMetadata": {"HTTPStatusCode": HTTPStatus.BAD_REQUEST}
        }
        self.assertEqual(result, expected_result)
    ...
    @patch('challenges.aws_utils.settings', DEBUG=False)
    @patch('challenges.aws_utils.get_boto3_client')
    def test_scale_resources_deregister_success(self, mock_get_boto3_client, mock_settings):
        ...
        with patch('challenges.utils.get_aws_credentials_for_challenge') as mock_get_aws_credentials_for_challenge, \
            ...
            mock_eval.return_value = {'some_key': 'some_value'}  # Use a dictionary here
            ...
            mock_client.update_service.return_value = {"ResponseMetadata": {"HTTPStatusCode": HTTPStatus.OK}}
    ...
    @patch('challenges.aws_utils.settings', DEBUG=False)
    @patch('challenges.aws_utils.get_boto3_client')
    def test_scale_resources_deregister_failure(self, mock_get_boto3_client, mock_settings):
        # Mock client and response
        mock_client = MagicMock()
        mock_get_boto3_client.return_value = mock_client
        mock_client.deregister_task_definition.side_effect = ClientError(
            {"Error": {"Message": "Scaling inactive workers not supported"}}, 'DeregisterTaskDefinition'
        )
    ...
    @patch('challenges.aws_utils.settings', DEBUG=False)
    @patch('challenges.aws_utils.get_boto3_client')
    def test_scale_resources_register_task_def_success(self, mock_get_boto3_client, mock_settings):
        # Mock client and response
        mock_client = MagicMock()
        mock_get_boto3_client.return_value = mock_client
    ...
    @patch('challenges.aws_utils.settings', DEBUG=False)
    @patch('challenges.aws_utils.get_boto3_client')
    def test_scale_resources_register_task_def_failure(self, mock_get_boto3_client, mock_settings):
        # Mock client and response
        mock_client = MagicMock()
        mock_get_boto3_client.return_value = mock_client
...
class TestDeleteWorkers(TestCase):
    ...
    @patch('challenges.aws_utils.settings', DEBUG=True)
    def test_delete_workers_in_dev_environment(self, mock_settings, mock_delete_log_group, mock_get_log_group_name, mock_delete_service_by_challenge_pk):
        # Mock a queryset
        ...
        # Assertions
        expected_failures = [
            {"message": "Workers cannot be deleted on AWS ECS service in development environment", "challenge_pk": 1},
            {"message": "Workers cannot be deleted on AWS ECS service in development environment", "challenge_pk": 2},
        ]
        self.assertEqual(result, {"count": 0, "failures": expected_failures})
    ...
    @patch('challenges.aws_utils.delete_log_group')
    @patch('challenges.aws_utils.settings', DEBUG=False)
    def test_delete_workers_no_workers(self, mock_settings, mock_delete_log_group, mock_get_log_group_name, mock_delete_service_by_challenge_pk):
        ...
        # Assertions
        expected_failures = [{"message": "Please select challenges with active workers only.", "challenge_pk": 1}]
        self.assertEqual(result, {"count": 0, "failures": expected_failures})
    ...
    @patch('challenges.aws_utils.delete_log_group')
    @patch('challenges.aws_utils.settings', DEBUG=False)
    def test_delete_workers_success(self, mock_settings, mock_delete_log_group, mock_get_log_group_name, mock_delete_service_by_challenge_pk):
        ...
        mock_delete_service_by_challenge_pk.return_value = {"ResponseMetadata": {"HTTPStatusCode": HTTPStatus.OK}}
        ...
        mock_delete_service_by_challenge_pk.assert_called_once_with(challenge=challenge_with_workers)
        mock_get_log_group_name.assert_called_once_with(challenge_with_workers.pk)
    ...
    @patch('challenges.aws_utils.delete_log_group')
    @patch('challenges.aws_utils.settings', DEBUG=False)
    def test_delete_workers_failure(self, mock_settings, mock_delete_log_group, mock_get_log_group_name, mock_delete_service_by_challenge_pk):
        ...
        # Assertions
        expected_failures = [{"message": "An error occurred", "challenge_pk": 1}]
        self.assertEqual(result, {"count": 0, "failures": expected_failures})
...
class TestRestartWorkers(TestCase):
    ...
    @patch('challenges.aws_utils.service_manager')
    @patch('challenges.aws_utils.settings', DEBUG=True)
    def test_restart_workers_in_dev_environment(self, mock_settings, mock_service_manager, mock_get_boto3_client):
        # Mock a queryset
        mock_queryset = [MagicMock(pk=1), MagicMock(pk=2)]
        ...
        # Assertions
        expected_failures = [
            {"message": "Workers cannot be restarted on AWS ECS service in development environment", "challenge_pk": 1},
            {"message": "Workers cannot be restarted on AWS ECS service in development environment", "challenge_pk": 2},
        ]
        self.assertEqual(result, {"count": 0, "failures": expected_failures})
    ...
    @patch('challenges.aws_utils.service_manager')
    @patch('challenges.aws_utils.settings', DEBUG=False)
    def test_restart_workers_docker_based_challenge(self, mock_settings, mock_service_manager, mock_get_boto3_client):
        # Mock a Docker-based challenge queryset
        challenge_docker_based = MagicMock(pk=1, is_docker_based=True, is_static_dataset_code_upload=False)
        ...
        # Assertions
        expected_failures = [
            {"message": "Sorry. This feature is not available for code upload/docker based challenges.", "challenge_pk": 1}
        ]
        self.assertEqual(result, {"count": 0, "failures": expected_failures})
    ...
    @patch('challenges.aws_utils.service_manager')
    @patch('challenges.aws_utils.settings', DEBUG=False)
    def test_restart_workers_no_workers(self, mock_settings, mock_service_manager, mock_get_boto3_client):
        # Mock a challenge with no workers
        challenge_no_workers = MagicMock(pk=1, workers=0, is_docker_based=False, is_static_dataset_code_upload=False)
    ...
    @patch('challenges.aws_utils.service_manager')
    @patch('challenges.aws_utils.settings', DEBUG=False)
    def test_restart_workers_success(self, mock_settings, mock_service_manager, mock_get_boto3_client):
        # Mock a challenge with active workers
        challenge_with_workers = MagicMock(pk=1, workers=2, is_docker_based=False, is_static_dataset_code_upload=False)
        mock_queryset = [challenge_with_workers]

        # Mock the service_manager response
        mock_service_manager.return_value = {"ResponseMetadata": {"HTTPStatusCode": HTTPStatus.OK}}
    ...
    @patch('challenges.aws_utils.service_manager')
    @patch('challenges.aws_utils.settings', DEBUG=False)
    def test_restart_workers_failure(self, mock_settings, mock_service_manager, mock_get_boto3_client):
        # Mock a challenge with active workers
        challenge_with_workers = MagicMock(pk=1, workers=2, is_docker_based=False, is_static_dataset_code_upload=False)
        mock_queryset = [challenge_with_workers]
...
class TestRestartWorkersSignalCallback(TestCase):
    @patch('challenges.aws_utils.settings', DEBUG=True)
    def test_restart_workers_signal_callback_debug_mode(self, mock_settings, *args):
        # Mock the sender and instance
        mock_sender = MagicMock()
        mock_instance = MagicMock()

        # Call the signal callback
        result = restart_workers_signal_callback(sender=mock_sender, instance=mock_instance, field_name="evaluation_script")
...
class TestGetLogsFromCloudwatch(TestCase):
    ...
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.settings', DEBUG=False)
    def test_get_logs_from_cloudwatch_success(self, mock_settings, mock_get_boto3_client):
    ...
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.settings', DEBUG=False)
    def test_get_logs_from_cloudwatch_resource_not_found(self, mock_settings, mock_get_boto3_client):
    ...
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.settings', DEBUG=False)
    def test_get_logs_from_cloudwatch_other_exception(self, mock_settings, mock_get_boto3_client, mock_logger):
    ...
    @patch('challenges.aws_utils.settings')  # Adjust the path to the actual module
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.logger')
    def test_delete_log_group_debug_mode(self, mock_logger, mock_get_boto3_client, mock_settings):
    ...
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.logger')
    def test_delete_log_group_non_debug_mode(self, mock_logger, mock_get_boto3_client, mock_settings):
        ...
        mock_client.delete_log_group.assert_called_once_with(logGroupName='test-log-group')
    ...
    @patch('challenges.aws_utils.get_boto3_client')
    @patch('challenges.aws_utils.logger')
    def test_delete_log_group_with_exception(self, mock_logger, mock_get_boto3_client, mock_settings):
        ...
        self.assertTrue('Delete failed' in str(args[0]), f"Expected 'Delete failed' in {args[0]}")
...
class TestCreateEKSNodegroup(unittest.TestCase):
    ...
    def test_create_eks_nodegroup_success(self, mock_client_token_generator, mock_create_service_by_challenge_pk, mock_construct_and_send_eks_cluster_creation_mail, mock_logger, mock_settings, mock_deserialize, mock_get_aws_credentials_for_challenge, mock_get_code_upload_setup_meta_for_challenge, mock_get_boto3_client):
        ...
        mock_get_code_upload_setup_meta_for_challenge.return_value = mock_cluster_meta
        mock_aws_credentials = {'AWS_ACCOUNT_ID': 'x', 'AWS_ACCESS_KEY_ID': 'x', 'AWS_SECRET_ACCESS_KEY': 'x', 'AWS_REGION': 'us-east-1', 'AWS_STORAGE_BUCKET_NAME': 'evalai-s3-bucket'}
        mock_get_aws_credentials_for_challenge.return_value = mock_aws_credentials
    ...
    @patch('challenges.aws_utils.settings')
    @patch('challenges.aws_utils.logger')
    def test_create_eks_nodegroup_client_error(self, mock_logger, mock_settings, mock_deserialize, mock_get_aws_credentials_for_challenge, mock_get_code_upload_setup_meta_for_challenge, mock_get_boto3_client):
        ...
        mock_get_code_upload_setup_meta_for_challenge.return_value = mock_cluster_meta
        mock_aws_credentials = {'AWS_ACCESS_KEY_ID': 'x', 'AWS_SECRET_ACCESS_KEY': 'x', 'AWS_REGION': 'us-east-1'}
        mock_get_aws_credentials_for_challenge.return_value = mock_aws_credentials
        ...
        # Assertions
        mock_get_boto3_client.assert_called_once_with("eks", mock_aws_credentials)
        ...
        self.assertIn(expected_message, str(actual_error), f"Expected message '{expected_message}' in {str(actual_error)}")
...
class TestSetupEksCluster(TestCase):
    ...
    @patch('challenges.aws_utils.logger')
    def test_setup_eks_cluster_success(self, mock_logger, mock_serializer, mock_get_cluster, mock_deserialize, mock_boto3, mock_get_aws):
    ...
    @patch('challenges.aws_utils.serializers.deserialize')
    @patch('challenges.aws_utils.logger')
    def test_setup_eks_cluster_create_role_failure(self, mock_logger, mock_deserialize, mock_boto3, mock_get_aws):
        """ Test case to simulate failure during EKS role creation """

        # Mocks
        mock_client = MagicMock()
        mock_client.create_role.side_effect = ClientError({"Error": {"Code": "SomeError"}}, "CreateRole")
        mock_boto3.return_value = mock_client
    ...
    @patch('challenges.aws_utils.serializers.deserialize')
    @patch('challenges.aws_utils.logger')
    def test_setup_eks_cluster_attach_role_policy_failure(self, mock_logger, mock_deserialize, mock_boto3, mock_get_aws):
        """ Test case to simulate failure during policy attachment """

        # Mocks
        mock_client = MagicMock()
        mock_client.attach_role_policy.side_effect = ClientError({"Error": {"Code": "SomeError"}}, "AttachRolePolicy")
        mock_boto3.return_value = mock_client
    ...
    @patch('challenges.aws_utils.serializers.deserialize')
    @patch('challenges.aws_utils.logger')
    def test_setup_eks_cluster_create_policy_failure(self, mock_logger, mock_deserialize, mock_boto3, mock_get_aws):
        """ Test case to simulate failure during custom ECR policy creation """

        # Mocks
        mock_client = MagicMock()
        mock_client.create_policy.side_effect = ClientError({"Error": {"Code": "SomeError"}}, "CreatePolicy")
        mock_boto3.return_value = mock_client
    ...
    @patch('challenges.serializers.ChallengeEvaluationClusterSerializer')
    @patch('challenges.models.ChallengeEvaluationCluster.objects.get')
    def test_setup_eks_cluster_serialization_failure(self, mock_get_cluster, mock_serializer, mock_logger, mock_deserialize, mock_boto3, mock_get_aws):
    ...
    @patch('challenges.serializers.ChallengeEvaluationClusterSerializer')
    @patch('challenges.models.ChallengeEvaluationCluster.objects.get')
    def test_setup_eks_cluster_subnets_creation(self, mock_get_cluster, mock_serializer, mock_create_subnets, mock_logger, mock_deserialize, mock_boto3, mock_get_aws):
...
class TestSetupEC2(TestCase):
    ...
    def test_setup_ec2_with_existing_instance(self, mock_deserialize, mock_create_ec2, mock_start_ec2):
        # Setup mock behavior
        mock_obj = MagicMock()
    ...
    @patch("challenges.aws_utils.create_ec2_instance")
    @patch("django.core.serializers.deserialize")
    def test_setup_ec2_without_existing_instance(self, mock_deserialize, mock_create_ec2, mock_start_ec2):
        # Setup mock behavior
        mock_obj = MagicMock()
        mock_obj.object = self.challenge
        mock_deserialize.return_value = [mock_obj]
    ...
    @patch('django.core.serializers.deserialize')
    def test_update_sqs_retention_period_task(self, mock_deserialize, mock_update_sqs_retention_period):
        ...
        mock_update_sqs_retention_period.assert_called_once_with(mock_challenge_obj)
