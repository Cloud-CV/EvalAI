...
class TestUtils(unittest.TestCase):
    ...
    def test_calculate_distinct_sorted_leaderboard_data(self, mock_filter, mock_exclude):
        mock_user = MagicMock()
        ...
        leaderboard_data, status_code = calculate_distinct_sorted_leaderboard_data(
            mock_user, mock_challenge_obj, mock_challenge_phase_split, True, 'order_by'
        )
...
class TestHandleSubmissionResume(unittest.TestCase):
    ...
    def test_handle_submission_resume(self, mock_requests_get, mock_timezone_now, mock_SubmissionSerializer):
        # Set up mock objects
        ...
        mock_response.json.return_value = {"submitted_image_uri": "http://example.com/image_uri"}
        ...
        # Assert the results
        mock_SubmissionSerializer.assert_called_once_with(mock_submission, data={"status": "resumed"}, partial=True)
        ...
        mock_requests_get.assert_called_once_with('http://example.com/input_file')
        ...
        mock_requests_get.return_value.json.return_value = {"submitted_image_uri": "some_uri"}
    ...
    @mock.patch('jobs.utils.timezone.now')
    @mock.patch('jobs.utils.requests.get')
    def test_handle_submission_rerun(self, mock_requests_get, mock_timezone_now, mock_SubmissionSerializer):
        # Set up mock objects
        mock_submission = mock.Mock()
        mock_submission.challenge_phase.challenge.is_docker_based = True
        mock_submission.challenge_phase.challenge.is_static_dataset_code_upload = False
        ...
        self.assertIsNone(message["submission_pk"])  # Adjusted to check for None
...
class TestGetLeaderboardDataModel(TestCase):
    ...
    def test_get_leaderboard_data_model(self, mock_get):
        ...
        # Act
        result = get_leaderboard_data_model(submission_pk, challenge_phase_split_pk)
...
class TestReorderSubmissionsComparator(TestCase):
    ...
    def setUp(self):
        ...
        self.leaderboard.schema = {
            ...
            "labels": ["score", "time"],
            "metadata": {
                "score": {"sort_ascending": False}
            }
        }
        self.challenge_obj.creator.get_all_challenge_host_email.return_value = ["host@example.com"]
        ...
        self.challenge_phase_split.show_leaderboard_by_latest_submission = False

    @patch('challenges.models.LeaderboardData.objects')
    @patch('hosts.utils.is_user_a_staff_or_host')
    def test_banned_email_ids(self, mock_is_user_a_staff_or_host, mock_leaderboard_data_objects):
        ...
        mock_leaderboard_data_objects.exclude.return_value.filter.return_value.order_by.return_value = [
            {"submission__participant_team": 1, "error": None, "filtering_score": 10, "result": {"score": 10, "time": 5}}
        ]
        ParticipantTeam = Mock()
        ParticipantTeam.objects.get.return_value.get_all_participants_email.return_value = ["banned@example.com"]
        ...
        with patch('challenges.models.ParticipantTeam', ParticipantTeam):
            result, status_code = calculate_distinct_sorted_leaderboard_data(
                self.user, self.challenge_obj, self.challenge_phase_split, False, "score"
            )
    ...
    @patch('challenges.models.LeaderboardData.objects')
    @patch('hosts.utils.is_user_a_staff_or_host')
    def test_team_in_all_banned_participant_team(self, mock_is_user_a_staff_or_host, mock_leaderboard_data_objects):
        mock_is_user_a_staff_or_host.return_value = False
        mock_leaderboard_data_objects.exclude.return_value.filter.return_value.order_by.return_value = [
            {"submission__participant_team": 1, "error": None, "filtering_score": 10, "result": {"score": 10, "time": 5}}
        ]
...
def reorder_submissions_comparator(submission_1, submission_2):
    ...
    if (
        ...
        return (submission_1.submitted_at > submission_2.submitted_at) - (submission_1.submitted_at < submission_2.submitted_at)
    return (submission_1.submitted_at < submission_2.submitted_at) - (submission_1.submitted_at > submission_2.submitted_at)
