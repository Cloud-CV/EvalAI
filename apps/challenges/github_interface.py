...
class GithubInterface:
    """
    Interface to communicate with GitHub API for bi-directional sync.
    Handles reading and updating files in a GitHub repository.
    """
    ...
    def make_request(self, url, method, params=None, data=None):
    ...
    def get_content_from_path(self, path):
        """
        Gets the file content information in JSON format from the repository.
    ...
    def get_data_from_path(self, path):
        """
        Gets the file data in string format from the repository.
        Decodes the base64 content from get_content_from_path.
        """
    ...
    def update_content_from_path(self, path, content, changed_field=None):
        """
        Updates the file content in the repository at particular path.
    ...
    def update_data_from_path(self, path, data, changed_field=None):
        """
        Updates the file data at particular path with the provided string data.
        Encodes to base64 and calls update_content_from_path.
        """
    ...
    def update_challenge_config(self, challenge, changed_field):
        ...
        try:
            ...
            header_comment = (
                "# If you are not sure what all these fields mean, "
    ...
    def update_challenge_phase_config(self, challenge_phase, changed_field):
        ...
        try:
            ...
            if target_index is None:
                logger.error(
                    f"Phase with codename {getattr(challenge_phase, 'codename', None)} "
                    "not found"
                )
                return False
