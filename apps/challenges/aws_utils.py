...
def register_task_def_by_challenge_pk(client, queue_name, challenge):
    ...
    if execution_role_arn:
        ...
        message = "Please ensure that the TASK_EXECUTION_ROLE_ARN is appropriately passed as an environment variable."
        return {
            "Error": message,
            "ResponseMetadata": {"HTTPStatusCode": HTTPStatus.BAD_REQUEST},
        }
...
def create_service_by_challenge_pk(client, challenge, client_token):
    ...
    Returns:
    dict: The response returned by the create_service method from boto3. If unsuccessful, returns an error dictionary
    """
...
def update_service_by_challenge_pk(
    ...
    Returns:
    dict: The response returned by the update_service method from boto3. If unsuccessful, returns an error dictionary
    """
