...
class PurgeSubmissionsTest(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="host_user", email="host@test.com", password="password"
        )
        ...
        self.phase = ChallengePhase.objects.create(
            name="Phase 1", challenge=self.challenge
        )

        self.participant_team = ParticipantTeam.objects.create(
            team_name="Part Team", created_by=self.user
        )
    ...
    def test_purge_stuck_submissions(self):
        ...
        url = reverse(
            "challenges:purge_stuck_submissions", args=[self.challenge.id]
        )
    ...
    def test_purge_access_denied_for_non_host(self):
        ...
        attacker = User.objects.create_user(
            username="attacker", email="bad@test.com", password="password"
        )
