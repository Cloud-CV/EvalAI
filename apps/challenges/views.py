...
def add_participant_team_to_challenge(
    ...
    # Check if user profile is complete if needed
    if challenge.is_users_profile_complete and not is_user_profile_complete(
        request.user
    ):
        response_data = {
            "error": "Please complete your EvalAI profile to participate in the challenge",
            "challenge_id": int(challenge_pk),
            "participant_team_id": int(participant_team_pk),
        }
        return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
