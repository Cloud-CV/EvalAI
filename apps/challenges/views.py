...
def create_challenge_using_zip_file(request, challenge_host_team_pk):
    ...
    try:
        with transaction.atomic():
            ...
            existing_challenge_phases = ChallengePhase.objects.filter(
                challenge=challenge
            )
            existing_challenge_phase_ids = [
                str(challenge_phase.config_id)
                for challenge_phase in existing_challenge_phases
            ]
            challenge_phases_data_ids = [
                str(challenge_phase_data["id"])
                for challenge_phase_data in challenge_phases_data
            ]
            challenge_phase_ids_to_delete = list(
                set(existing_challenge_phase_ids)
                - set(challenge_phases_data_ids)
            )
            for challenge_phase_id_to_delete in challenge_phase_ids_to_delete:
                challenge_phase = ChallengePhase.objects.filter(
                    challenge__pk=challenge.pk,
                    config_id=challenge_phase_id_to_delete,
                ).first()
            ...
            existing_challenge_phase_splits = (
    ...
    if not len(error_messages):
        if not challenge_queryset:
            ...
            try:
                ...
                for (
                    challenge_phase_id_to_delete
                ) in challenge_phase_ids_to_delete:
                    challenge_phase = ChallengePhase.objects.filter(
                        challenge__pk=challenge.pk,
                        config_id=challenge_phase_id_to_delete,
                    ).first()
