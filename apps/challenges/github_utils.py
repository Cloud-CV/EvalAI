...
def github_challenge_sync(challenge_id, changed_field):
    ...
    try:
        ...
        if not isinstance(changed_field, str):
            logger.error(
                f"Invalid changed_field type: {type(changed_field)}, expected string"
            )
            return False
        ...
        if not challenge.github_repository or not challenge.github_token:
            logger.warning(
                f"Challenge {challenge_id} missing GitHub configuration"
            )
            return False
        ...
        github_interface = GithubInterface(
            ...
            or "challenge",  # Default to 'challenge' branch
            challenge.github_token,
        )

        # Update challenge config in GitHub with the specific changed field
        success = github_interface.update_challenge_config(
            challenge, changed_field
        )
...
def github_challenge_phase_sync(challenge_phase_id, changed_field):
    ...
    try:
        ...
        if not challenge.github_repository or not challenge.github_token:
            logger.warning(
                f"Challenge {challenge.id} missing GitHub configuration"
            )
            return False
        ...
        success = github_interface.update_challenge_phase_config(
        ...
        if success:
            ...
            logger.error(
                f"Failed to sync challenge phase {challenge_phase_id} to GitHub"
            )
            return False
