...
def load_challenge():
    ...
    try:
        ...
        logger.exception("Challenge with queue name %s does not exists" % (QUEUE_NAME))
        raise
...
def process_submission_message(message):
    ...
    run_submission(
        challenge_pk, challenge_phase, submission_instance, user_annotation_file_path
    )
...
def extract_submission_data(submission_pk):
    ...
    submission_input_file_path = SUBMISSION_INPUT_FILE_PATH.format(
        submission_id=submission.get("id"), input_file=submission_input_file_name
    )
...
def run_submission(
    ...
    * Calls evaluation script to evaluate the particular submission
    ...
    submission_data = {"submission_status": "running", "submission": submission_pk}
...
def main():
    ...
    while True:
        logger.info("Fetching new messages from the queue {}".format(QUEUE_NAME))
        ...
        if message_body:
            ...
            if submission:
                if submission.get("status") == "finished":
                    ...
                    message_receipt_handle = message.get("receipt_handle")
                    logger.info("Processing message body: {}".format(message_body))
