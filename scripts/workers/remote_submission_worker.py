...
def process_submission_callback(body, api):
    try:
        w_u.logger.info("[x] Received submission message %s" % body)
        process_submission_message(body, api)
...
def process_submission_message(message, api):
    """
    ...
    submission_instance = extract_submission_data(submission_pk, api)
...
def extract_submission_data(submission_pk, api):
    """
    ...
    if not submission:
        w_u.logger.critical("Submission {} does not exist".format(submission_pk))
        traceback.print_exc()
