...
- **Backend**: `docker exec workspace-django-1 bash -c 'cd /code && python -m ruff check --no-fix apps/ && python -m ruff format --check apps/'` (config in `pyproject.toml`; drop `--no-fix`/`--check` to apply fixes)
