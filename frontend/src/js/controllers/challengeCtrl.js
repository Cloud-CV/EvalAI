...
(function() {
    ...
    function ChallengeCtrl(utilities, loaderService, $scope, $state, $http, $stateParams, $rootScope, Upload, $interval, $mdDialog, moment) {
      ...
      // loader for existing teams
        ...
        // function to get filtered submissions list
        vm.getFilteredSubmissionResults = function() {

            if (vm.phaseId){
                if (vm.filterQuery === ''){
                    vm.getAllSubmissionResults(vm.phaseId);
                }
                else{
                    parameters.url = "jobs/" + vm.challengeId + "/challenge_phase/" + vm.phaseId + "/filter_query/" + vm.filterQuery + "/filter_by/" + vm.filterBy;
                    parameters.method = 'GET';
                    parameters.data = {};
                    parameters.callback = {
                        onSuccess: function(response) {
            ...
            else if (vm.filterQuery != undefined && vm.filterQuery != '') {
                $rootScope.notify('error', 'Please, first select a phase to get list of submissions!');
            }
        }
