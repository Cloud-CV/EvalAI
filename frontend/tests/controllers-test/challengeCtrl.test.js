...
describe("Unit tests for challenge controller", function () {
  ...
  describe("Unit tests for all the global backend calls", function () {
    ...
    var challengePhaseSplitSuccess,
      participantTeamChallengeSuccess,
      participantTeamSuccess,
      selectExistTeamSuccess;
    ...
    beforeEach(function () {
      ...
      utilities.sendRequest = function (parameters) {
        ...
        if (
          ...
          parameters.url == "challenges/challenge/undefined/"
        ) {
          successResponse = challengeSuccessResponse;
        } else if (
          participantTeamChallengeSuccess == true &&
          parameters.url ==
            "participants/participant_teams/challenges/undefined/user"
        ) {
        ...
        if (
          (challengePhaseSuccess == true &&
            parameters.url ==
              "challenges/challenge/undefined/challenge_phase") ||
          (challengeSuccess == true &&
            parameters.url == "challenges/challenge/undefined/") ||
          (challengePhaseSplitSuccess == true &&
            parameters.url == "challenges/undefined/challenge_phase_split") ||
          (participantTeamChallengeSuccess == true &&
            parameters.url ==
              "participants/participant_teams/challenges/undefined/user") ||
          (participantTeamSuccess == true &&
          ...
          (selectExistTeamSuccess == true &&
            parameters.url ==
              "challenges/challenge/undefined/participant_team/null")
        ) {
          ...
          (challengePhaseSplitSuccess == false &&
            parameters.url == "challenges/undefined/challenge_phase_split") ||
          (participantTeamChallengeSuccess == false &&
            parameters.url ==
              "participants/participant_teams/challenges/undefined/user") ||
          (participantTeamSuccess == false &&
            parameters.url == "participants/participant_team") ||
    ...
    it("get details of challenges corresponding to participant teams of that user \
      ...
      vm = createController();
      expect(vm.currentDate).toEqual(
        participantTeamChallengeSuccessResponse.datetime_now
      );
        ...
        function () {
          ...
          if (participantTeamSuccessResponse.next == null) {
            ...
            expect(vm.currentPage).toEqual(
              participantTeamSuccessResponse.next.split("page=")[1] - 1
            );
          }
    ...
    it("success of selectExistTeam function \
      ...
      expect($state.go).toHaveBeenCalledWith(
        "web.challenge-main.challenge-page.submission"
      );
    ...
    it("get details of the particular challenge phase `challenges/challenge/<challenge_id>/challenge_phase`", function () {
      ...
      for (var i = 0; i < successResponse.results.length; i++) {
        var offset = new Date(
          successResponse.results[i].start_date
        ).getTimezoneOffset();
        expect(vm.phases.results[i].time_zone).toEqual(
          moment.tz.zone(timezone).abbr(offset)
        );
      }
      expect(utilities.hideLoader).toHaveBeenCalled();
    });
    ...
    it("backend error of particular challenge phase `challenges/challenge/<challenge_id>/challenge_phase`", function () {
      ...
      spyOn(utilities, "hideLoader");
      vm = createController();
      expect(utilities.storeData).toHaveBeenCalledWith(
        "emailError",
  ...
  describe("Unit tests for displayDockerSubmissionInstructions function \
    ...
    it("when submission limit exceeded", function () {
      ...
      for (var i = 0; i < details.length; i++) {
        expect(vm.phaseRemainingSubmissionsFlags[details[i].id]).toEqual(
          "maxExceeded"
        );
      }
      expect(utilities.hideLoader).toHaveBeenCalled();
    ...
    it("when submission limit exceeded & todays remaining submission count less than 0", function () {
      ...
      for (var i = 0; i < details.length; i++) {
        ...
        expect(vm.hoursLeft).toEqual(
          Math.floor(vm.remainingTime - vm.days * 86400)
        );
        expect(vm.hours).toEqual(Math.floor(vm.hoursLeft / 3600));
        expect(vm.minutesLeft).toEqual(
          Math.floor(vm.hoursLeft - vm.hours * 3600)
        );
        expect(vm.minutes).toEqual(Math.floor(vm.minutesLeft / 60));
        if (vm.remainingTime === 0) {
          expect(vm.phaseRemainingSubmissionsFlags[vm.eachPhase.id]).toEqual(
            "showSubmissionNumbers"
          );
        }
      }
  ...
  describe("Unit tests for makeSubmission function \
    ...
    it("succesfully submission", function () {
      ...
      expect($rootScope.notify).toHaveBeenCalledWith(
        "success",
        "Your submission has been recorded succesfully!"
      );
      expect(vm.stopLoader).toHaveBeenCalled();
    });

    it("forces isPublicSubmission=true when disable_private_submission is true", function () {
      success = true;
      vm.isParticipated = true;
      vm.eligible_to_submit = true;
      vm.disable_private_submission = true;
      // simulate file‐upload path so it doesn’t short‐circuit on URL logic
      vm.isSubmissionUsingUrl = false;
      vm.input_file = new Blob(["x"], { type: "text/plain" });
      vm.currentPhaseAllowedSubmissionFileTypes = ["txt"];

      // capture the outgoing FormData
      var originalSend = utilities.sendRequest;
      var captured;
      spyOn(window, "FormData").and.callFake(function () {
        var f = { store: {} };
        f.append = function (k, v) {
          f.store[k] = v;
        };
        return f;
      });
      utilities.sendRequest = function (params) {
        captured = params.data.store;
        // call success path so post‐submission cleanup happens
        params.callback.onSuccess({ status: 200, data: successResponse });
      };

      vm.makeSubmission();

      expect(captured.is_public).toBe(true, "should set is_public to true");

      // restore
      utilities.sendRequest = originalSend;
    });

    it("respects vm.isPublicSubmission when disable_private_submission is false", function () {
      success = true;
      vm.isParticipated = true;
      vm.eligible_to_submit = true;
      vm.disable_private_submission = false;
      vm.isPublicSubmission = false; // developer explicitly said “private”
      vm.isSubmissionUsingUrl = false;
      vm.input_file = new Blob(["x"], { type: "text/plain" });
      vm.currentPhaseAllowedSubmissionFileTypes = ["txt"];
      ...
      expect(captured.is_public).toBe(
        false,
        "should use the vm.isPublicSubmission value"
      );
  ...
  describe("Unit tests for getResults function", function () {
    ...
    it("get the submission count \
      ...
      vm.getResults(phaseId);
      expect(vm.submissionCount).toEqual(
        successResponse.participant_team_submission_count
      );
    });
    ...
    submission_list.forEach((response) => {
      it(
        "get submissions of a particular challenge phase when pagination next is " +
          response.next +
          " \
                and previous is " +
          response.previous +
          "`jobs/challenge/<challenge_id>/challenge_phase/<phase_id>/submission/`",
        function () {
          submissionListSuccess = true;
          var phaseId = 1;
          ...
          for (var i = 0; i < successResponse.results.length; i++) {
            expect(
              vm.submissionVisibility[successResponse.results[i].id]
            ).toEqual(successResponse.results[i].is_public);
            expect(
              (vm.baselineStatus[successResponse.results[i].id] =
                successResponse.results[i].is_baseline)
            );
          }
          ...
          if (vm.submissionResult.next == null) {
            ...
            expect(vm.isNext).toEqual("");
            expect(vm.currentPage).toEqual(
              vm.submissionResult.next.split("page=")[1] - 1
            );
          }
  ...
  describe("Unit tests for refreshSubmissionData function \
    ...
    paginated_list.forEach((response) => {
      it(
        "submission data list have count" +
          response.count +
          ", next " +
          response.next +
          "and previous " +
        ...
        function () {
          ...
          for (var i = 0; i < response.results.length; i++) {
            expect(vm.submissionVisibility[response.results[i].id]).toEqual(
              response.results[i].is_public
            );
            expect(vm.baselineStatus[response.results[i].id]).toEqual(
              response.results[i].is_baseline
            );
          }
      ...
      it(
        ...
        function () {
          ...
          if (vm.existTeam.next == null) {
            ...
            expect(vm.isNext).toEqual("");
            expect(vm.currentPage).toEqual(
              vm.existTeam.next.split("page=")[1] - 1
            );
          }
  ...
  describe("Unit tests for showapprovalparticipantteamDialog function", function () {
    ...
    it("should open dialog when approved_status is true", function () {
      ...
      vm.showapprovalparticipantteamDialog(
        challengeId,
        participant_team_id,
        approved_status
      );
    ...
    it("should call check_approval_status when approved_status is false", function () {
      ...
      expect(vm.check_approval_status).toHaveBeenCalledWith(
        challengeId,
        participant_team_id,
        approved_status,
        false
  ...
  describe("Unit tests for check_approval_status function", function () {
    ...
    it("should handle successful approval of participant team", function () {
      ...
      expect($rootScope.notify).toHaveBeenCalledWith(
        "success",
        "Participant Team Approved successfully."
      );
      expect($mdDialog.hide).toHaveBeenCalled();
    });
    ...
    it("should handle disapproval of participant team", function () {
      ...
      expect($rootScope.notify).toHaveBeenCalledWith(
        "success",
        "Participant Team Disapproved successfully."
      );
  ...
  describe("Unit tests for showRemainingSubmissions function \
    ...
    it("when submission limit exceeded", function () {
      ...
      expect(vm.maxExceededMessage).toEqual(
        successResponse.phases.first_object.limits.message
      );
    });
    ...
    it("when today remaining submission greater than 0", function () {
      ...
      vm.showRemainingSubmissions();
      expect(vm.remainingSubmissions).toEqual(
        successResponse.phases.first_object.limits
      );
    ...
    it("countdown for the remaining timer", function () {
      ...
      expect(vm.remainingTime).toEqual(
        successResponse.phases.first_object.limits.remaining_time
      );
      expect(vm.days).toEqual(Math.floor(vm.remainingTime / 24 / 60 / 60));
      expect(vm.hoursLeft).toEqual(
        Math.floor(vm.remainingTime - vm.days * 86400)
  ...
  describe("Unit tests for showMdDialog", function () {
    it("Variables initialized", function () {
      ...
      expect(vm.method_description).toEqual(
        vm.submissionMetaData.method_description
      );
  ...
  describe("Unit tests for editChallengeOverview function \
    ...
    it("valid editchallengeoverview form and succesfull edit", function () {
      ...
      expect($rootScope.notify).toHaveBeenCalledWith(
        "success",
        "The description is successfully updated!"
      );
    });
  ...
  describe("Unit test for editChallengeTag function", function () {
    ...
    it("valid edit challenge tag", function () {
      ...
      expect($rootScope.notify).toHaveBeenCalledWith(
        "success",
        "The challenge tags and domain is successfully updated!"
      );
    ...
    it("valid edit challenge more than 4 error", function () {
      ...
      expect($rootScope.notify).toHaveBeenCalledWith(
        "error",
        "Invalid tags! Maximum 4 tags are allowed, and each tag must be 15 characters or less."
      );
    });
  });
  ...
  describe("Unit tests for deleteChallenge function \
    ...
    it("valid delete challenge form & successfully delete challenge", function () {
      ...
      expect($rootScope.notify).toHaveBeenCalledWith(
        "success",
        "The Challenge is successfully deleted!"
      );
    });
  ...
  describe("Unit tests for submissionGuidelinesDialog function", function () {
    it("open dialog to `edit submission guidelines`", function () {
      ...
      expect(vm.tempSubmissionGuidelines).toEqual(
        vm.page.submission_guidelines
      );
      expect($mdDialog.show).toHaveBeenCalled();
      expect($mdDialogOpened).toEqual(true);
    });
  ...
  describe("Unit tests for editSubmissionGuideline function \
    ...
    it("valid edit submission guidelines form & successfull edit", function () {
      ...
      expect($rootScope.notify).toHaveBeenCalledWith(
        "success",
        "The submission guidelines is successfully updated!"
      );
    });
    ...
    it("valid edit submission guidelines form & backend error", function () {
      ...
      expect(utilities.getData).toHaveBeenCalledWith("challengeCreator");
      expect($mdDialog.hide).toHaveBeenCalled();
      expect(vm.page.submission_guidelines).toEqual(
        vm.tempSubmissionGuidelines
      );
      expect($rootScope.notify).toHaveBeenCalledWith("error", errorResponse);
    });
  ...
  describe("Unit tests for editEvaluationCriteria function \
    ...
    it("valid `edit evaluation criteria` form & successfull edit", function () {
      ...
      expect($rootScope.notify).toHaveBeenCalledWith(
        "success",
        "The evaluation details is successfully updated!"
      );
    });
  ...
  describe("Unit tests for editEvalScript function \
    ...
    it("valid `edit evaluation script` form & successfull edit", function () {
      ...
      expect($rootScope.notify).toHaveBeenCalledWith(
        "success",
        "The evaluation script is successfully updated!"
      );
    });
    ...
    it("invalid `edit evaluation script` form & frontend error", function () {
      ...
      expect($rootScope.notify).toHaveBeenCalledWith(
        "error",
        "Please upload a valid evaluation script!"
      );
    });
  ...
  describe("Unit tests for editTermsAndConditions function \
    ...
    it("valid `edit terms and conditions` form & successfull edit", function () {
      ...
      expect($rootScope.notify).toHaveBeenCalledWith(
        "success",
        "The terms and conditions are successfully updated!"
      );
    });
  ...
  describe("Unit tests for editChallengeTitle function \
    ...
    it("valid `edit terms and conditions` form & successfull edit", function () {
      ...
      expect($rootScope.notify).toHaveBeenCalledWith(
        "success",
        "The challenge title is  successfully updated!"
      );
    });
  ...
  describe("Unit tests for challengePhaseDialog function", function () {
    it("open dialog to `edit challenge phase`", function () {
      ...
      expect(vm.page.max_submissions_per_day).toEqual(
        phase.max_submissions_per_day
      );
      expect(vm.page.max_submissions_per_month).toEqual(
        phase.max_submissions_per_month
      );
  ...
  describe("Unit tests for editChallengePhase function \
    ...
    it("valid `edit challenge phase` form & successfull edit", function () {
      ...
      expect($rootScope.notify).toHaveBeenCalledWith(
        "success",
        "The challenge phase details are successfully updated!"
      );
